...
export const getLocale = (language) => {
  const locales = {
    zh: {
      ...
      successDownload: (pkg, date, data) =>
        `${pkg} 模块 ${date}下载量 (${data.start} 至 ${data.end}) 为 ${data.downloads} 次`,
    },
    en: {
      ...
      successDownload: (pkg, date, data) =>
        `${pkg} module downloads for the past ${date} (${data.start} - ${data.end}) were ${data.downloads} times`,
    },
  };
